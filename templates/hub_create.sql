...
BEGIN
    ...
    v_hub_rk_2 := -2;
        ...
        IF v_res > 0 THEN
            EXECUTE 'SELECT COUNT(*) FROM {{hub.hub_schema}}.{{hub.hub_name_only}} WHERE {{ hub.name }} = ' || v_hub_rk_1
                INTO v_res;
            ...
            EXECUTE 'SELECT COUNT(*) FROM {{hub.hub_schema}}.{{hub.hub_name_only}} WHERE {{hub.name}} = ' || v_hub_rk_2
                INTO v_res;
