...
#### Список 4 (11.02.2024)
* Реализована возможность создавать разные конфигурации для разных "источников".
Для этого в отдельный каталог необходимо скопировать:
  * Файл настроек generator.yaml 
  * BAT-файл start_process.bat 
  * В bat-файле прописать пути до файлов activate.bat, main.py, generator.yaml
  * Выполнить "точную" настройку generator.yaml для Вашего источника.
  Обязательно прописать полный путь к каталогу с шаблонами.
  * При необходимости можно скопировать шаблоны в новый каталог, сделать необходимые изменения и прописать путь в generator.yaml. 
...
Данные (признаки) вносятся в поле колонки через запятую. Наличие пробелов или других символов (кроме самих признаков) исключается.  
Пример: 
  * 'pk,rk'
  * 'pk'
  * Значения в колонке могут быть следующие:
    * pk - отмечаются поля, входящие в состав "первичного ключа" таблицы и однозначно определяющие уникальную строку в разрезе историчности.  
    ...
    Поля *_ini НЕ надо отмечать в колонке Tgt_PK.
    * rk/bk - отмечаются поля *_rk, входящие в состав хабов.   
    Значение rk означает, что из источника передаются данные в поле *_rk,
    а поле *_id, будет сформировано (рассчитано) при загрузке данных. is_bk = false  
    Значение bk означает, что из источника передаются данные в поле *_id,
  ...
  * Добавлен "мягкий" контроль соответствия типов полей источника и целевой таблицы  
  ...
  Пара полей *_id / *_rk должна иметь типы 'string' / 'bigint'
